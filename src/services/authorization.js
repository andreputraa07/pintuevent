const accessRoles = new Set(["customer", "organizer", "admin"]);

export function normalizeRole(value) {
  return accessRoles.has(value) ? value : "customer";
}
...
export async function getAccessSession() {
  ...
  const { data: profile, error: profileError } = await supabase
    .from("profiles")
    .select("id,full_name,avatar_url,role,status,organizer_verified")
    .eq("id", user.id)
    .single();
...
export async function signIn(email, password, demoRole = "customer") {
  ...
  if (!isSupabaseConfigured) {
    ...
    const session = { id: `demo-${role}`, email, role, status: "active" };
