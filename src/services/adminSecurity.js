...
export function validateRefund(amount, paidAmount, alreadyRefunded = 0) {
  ...
  if (amount + alreadyRefunded > paidAmount) return "Refund melebihi jumlah pembayaran.";
  return null;
}
export function requireReason(reason) { return reason?.trim().length >= 5 ? null : "Alasan minimal 5 karakter."; }
